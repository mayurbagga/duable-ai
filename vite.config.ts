...
export default defineConfig((config) => {
  return {
    ...
    plugins: [
      nodePolyfills({
        include: ['buffer', 'process', 'util', 'stream', 'crypto', 'url', 'querystring'],
        globals: {
          Buffer: true,
          process: true,
          global: true,
      ...
      {
        name: 'fix-undici-util',
        resolveId(source) {
          if (source === 'node:util/types') {
            return 'util';
          }
          if (source === 'node:util') {
            return 'util';
      ...
      {
        name: 'handle-mock-service-worker',
        configureServer(server) {
          server.middlewares.use((req, res, next) => {
            if (req.url === '/mockServiceWorker.js') {
              res.setHeader('Content-Type', 'application/javascript');
              res.end('// Mock Service Worker - Not needed for this application');
              return;
            }
            next();
          });
