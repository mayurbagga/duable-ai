...
import { IointelligenceProvider } from './registry';
...
export class LLMManager {
  ...
  private constructor(_env: Record<string, string>) {
    this._provider = new IointelligenceProvider();
    this._env = _env;
  }
  ...
  async updateModelList(options: {
    ...
    if (!apiKeys || !apiKeys['iointelligence']) {
      return this.getStaticModelList();
    }

    try {
      const dynamicModels =
        (await this._provider.getDynamicModels?.(apiKeys, providerSettings?.['iointelligence'], serverEnv)) || [];
      ...
      console.log(`Error getting dynamic models from Iointelligence:`, err);
      return this.getStaticModelList();
    }
  }
  ...
  async getModelListFromProvider(
    ...
    // Support Iointelligence provider
    if (providerArg.name !== 'iointelligence') {
      throw new Error(`Only Iointelligence provider is supported`);
    }
